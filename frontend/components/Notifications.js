    ...
    <div className="max-w-[700px] mx-auto space-y-6">
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
        ...
        <button
          onClick={handleSave}
          disabled={loading}
          className="w-full mt-6 py-3.5 bg-primary text-white dark:bg-primary-600 rounded-xl font-semibold hover:bg-primary/90 dark:hover:bg-primary-700 active:scale-[0.98] transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-primary/30 dark:shadow-none"
        >
...
function ToggleRow({ label, description, enabled, onChange, icon }) {
  return (
    <div className="flex justify-between items-center py-3 sm:py-4 border-b border-gray-100 last:border-0">
      <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
        <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl bg-gray-50 flex items-center justify-center text-lg sm:text-xl flex-shrink-0">
          {icon}
        </div>
        <div className="min-w-0 flex-1">
          <div className="text-sm sm:text-base font-semibold text-dark truncate">{label}</div>
          <div className="text-xs text-gray-400 truncate hidden sm:block">{description}</div>
        </div>
      </div>
      <button
        onClick={() => onChange(!enabled)}
        className={`px-3 sm:px-5 py-1.5 sm:py-2.5 rounded-full text-xs sm:text-sm font-semibold transition-all flex-shrink-0 ml-2 sm:ml-0 ${
          enabled
