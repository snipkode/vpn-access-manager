    ...
    <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {plans.map((plan) => (
              ...
              >
                ...
                <div className="text-2xl font-bold text-[#007AFF] mb-1 tracking-tight">
                  {formatCurrency(plan.price)}
                </div>
                <div className="text-[11px] text-gray-400 font-medium">{plan.duration_days} hari</div>
                {selectedPlan === plan.id && (
                  <div className="mt-2 text-[10px] text-[#007AFF] font-bold uppercase tracking-wide">
      ...
      <div>
        ...
        <input
          ...
          className="w-full px-4 py-[14px] bg-gray-50 border border-gray-200 rounded-2xl text-dark text-[17px] font-semibold focus:border-[#007AFF] focus:ring-2 focus:ring-[#007AFF]/20 outline-none transition-all"
          min="10000"
          step="10000"
          required
        />
      ...
      <div>
        <label className="block text-[11px] font-semibold text-gray-400 uppercase tracking-wider mb-2">
          <i className="fas fa-university mr-1.5 text-[#007AFF]" />
        ...
        <input
          ...
          className="w-full px-4 py-[14px] bg-gray-50 border border-gray-200 rounded-2xl text-dark text-[17px] focus:border-[#007AFF] focus:ring-2 focus:ring-[#007AFF]/20 outline-none transition-all placeholder:text-gray-300"
          required
        />
      </div>
      ...
      <div>
        <label className="block text-[11px] font-semibold text-gray-400 uppercase tracking-wider mb-2">
          <i className="fas fa-calendar mr-1.5 text-[#007AFF]" />
          Tanggal Transfer
        </label>
        <input
          type="date"
          ...
          className="w-full px-4 py-[14px] bg-gray-50 border border-gray-200 rounded-2xl text-dark text-[17px] focus:border-[#007AFF] focus:ring-2 focus:ring-[#007AFF]/20 outline-none transition-all"
          required
        />
      </div>
      ...
      <div>
        ...
        <div className="relative">
          <input
            ...
            className="w-full px-4 py-[14px] bg-gray-50 border border-gray-200 rounded-2xl text-dark text-[17px] focus:border-[#007AFF] focus:ring-2 focus:ring-[#007AFF]/20 outline-none transition-all file:mr-4 file:py-3 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-semibold file:bg-[#007AFF]/10 file:text-[#007AFF] hover:file:bg-[#007AFF]/20"
        ...
        {proofPreview && (
          <div className="mt-4 bg-gray-50 rounded-2xl p-3 text-center border border-gray-100">
            <img
              src={proofPreview}
              alt="Proof preview"
        ...
        {proofFile && !proofPreview && (
          <div className="mt-4 bg-gray-50 rounded-2xl p-4 flex items-center gap-3 border border-gray-100">
            <i className="fas fa-file-pdf text-red-500 text-3xl" />
            <div className="flex-1">
              <div className="text-sm font-semibold text-dark">{proofFile.name}</div>
              <div className="text-[11px] text-gray-400">{(proofFile.size / 1024 / 1024).toFixed(2)} MB</div>
            </div>
      ...
      <div>
        <label className="block text-[11px] font-semibold text-gray-400 uppercase tracking-wider mb-2">
          <i className="fas fa-sticky-note mr-1.5 text-[#007AFF]" />
          Catatan (Opsional)
        </label>
        <textarea
          value={notes}
          ...
          rows={3}
          className="w-full px-4 py-[14px] bg-gray-50 border border-gray-200 rounded-2xl text-dark text-[17px] focus:border-[#007AFF] focus:ring-2 focus:ring-[#007AFF]/20 outline-none transition-all resize-none placeholder:text-gray-300"
        />
      </div>
      ...
      <button
        type="submit"
        disabled={submitting}
        className="w-full py-[18px] bg-[#007AFF] text-white rounded-2xl font-semibold text-[17px] tracking-tight hover:bg-[#007AFF]/90 active:scale-[0.98] transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-[#007AFF]/30"
      >
        {submitting ? (
    ...
    <div className="mb-6">
      ...
      <div className="space-y-3">
        {bankAccounts.map((bank) => (
          <div key={bank.id} className="bg-gradient-to-br from-[#F2F2F7] to-white rounded-2xl p-5 border border-gray-100 shadow-sm">
            <div className="flex items-center gap-3 mb-3">
              <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center text-3xl shadow-sm">
                🏦
              </div>
              <div className="flex-1">
                <div className="text-[17px] font-bold text-dark tracking-tight">{bank.bank}</div>
                <div className="text-[13px] text-gray-500 font-medium">{bank.account_name}</div>
              </div>
            </div>
            <div className="bg-white rounded-xl p-4 mb-3 border border-gray-100">
              <div className="text-[11px] text-gray-400 font-semibold uppercase tracking-wider mb-1">Nomor Rekening</div>
              <div className="text-2xl font-mono font-bold text-dark tracking-tight">{bank.account_number}</div>
            </div>
            {bank.description && (
              <p className="text-[13px] text-gray-600 leading-relaxed">{bank.description}</p>
            )}
            {bank.qr_code_url && (
              <div className="mt-4 text-center bg-white rounded-2xl p-4 inline-block">
                <img
                  src={bank.qr_code_url}
                  alt="QR Code"
                  className="max-h-40 mx-auto rounded-xl shadow-sm"
                />
              </div>
            )}
          </div>
  ...
  if (!payments || payments.length === 0) {
    return (
      <div className="bg-[#F2F2F7] rounded-2xl p-12 text-center">
        ...
        <div className="text-[17px] font-semibold text-dark mb-1 tracking-tight">{emptyMessage}</div>
        <div className="text-[13px] text-gray-400 font-medium">Pembayaran yang dikirim akan muncul di sini</div>
      </div>
    );
  }
      ...
      {payments.map((payment) => (
        ...
        >
          <div className="flex justify-between items-start mb-3">
            <div>
              <div className="text-2xl font-bold text-[#007AFF] tracking-tight">
          ...
          <div className="grid grid-cols-2 gap-3 text-[13px]">
            ...
            {payment.admin_note && (
              <div className="col-span-2 bg-[#FFF3CD] border border-[#FFC107] rounded-xl p-3 mt-2">
                <div className="text-[12px] text-[#856404] font-medium">
