...
router.get('/config', async (req, res) => {
  try {
    ...
    // If no plans in subscription_plans, use default PLANS
    if (plans.length === 0) {
    ...
    res.json({
      billing_enabled: settings.billing_enabled || false,
      currency: settings.currency || 'IDR',
      min_topup: settings.min_topup || 10000,
      max_topup: settings.max_topup || 1000000,
      plans,
      bank_accounts,
    });
  } catch (error) {
    console.error('Get billing config error:', error.message);
    res.status(500).json({ error: 'Failed to get billing configuration', details: error.message });
  }
});
